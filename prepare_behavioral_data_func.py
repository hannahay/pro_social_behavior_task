...
def prepare_behavioral_data(data_folder, mouse, day, empathic_port, window, equal_trials, recipient_features, previous_choice):
    ...
    - empathic_port: int, indicator for the empathic port (0 or 1).
    ...
    file_name_actor = os.path.join(data_folder, f'{mouse}_day{day}.analysis.h5')
    ...
    R_ear_loc = locations_dict[:, indices['r_ear'], :]
    L_ear_loc = locations_dict[:, indices['l_ear'], :]
    nose_loc = locations_dict[:, indices['nose'], :]
    body_loc = locations_dict[:, indices['neck'], :]

    # Import the time (frames) of the trials with pokes
    filename = os.path.join(data_folder, f'{mouse}_day{day}_position.mat')
    mat = scipy.io.loadmat(filename)
    filename_ts = os.path.join(data_folder, f'{mouse}_day{day}_timeStamps.csv')
    ...
    dist_nose_to_divider = abs(nose_loc[:, 1] - ((div_pos_start[0] + div_pos_stop[1]) / 2))
    ...
    X[4] = np.cos(angle_nose_to_divider)
    X[5] = np.sin(angle_nose_to_divider)
    headers = ["dist nose to foodport", "dist nose to recip foodport", "dist nose to divider", "velocity",
               "cos nose to divider", "sin nose to divider"]

    if recipient_features:
        file_name_recip = os.path.join(data_folder, f'{mouse}_day{day}_recip.analysis.h5')
    ...
    if previous_choice:
        ...
        XX = np.vstack([XX, previous_port])
    
    port_label = [empathic_port] * len(labels)
    X_extended = np.vstack([XX, labels, port_label]).T
    
    if equal_trials:
        empathic_indices = np.where(X_extended[:, -2] == 'empathic')[0]
        selfish_indices = np.where(X_extended[:, -2] == 'selfish')[0]
